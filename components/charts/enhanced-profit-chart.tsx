...
export function EnhancedProfitChart({ data, bigHitDates = [] }: EnhancedProfitChartProps) {
  // Use all data instead of filtered data
  const displayData = data; // filterDataForDisplay(data)

  // Custom tooltip formatter
  const tooltipFormatter = (value: number, name: string) => {
    console.log("Tooltip formatter:", { value, name });
    return [formatCurrency(value), name === "profit" ? "Profit Acumulado" : "Profit Diário"]
  }
        ...
        >
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={displayData} margin={{ top: 10, right: 30, left: 0, bottom: 30 }}>
              ...
              {/* Reference lines for big hits */}
              {bigHitDates.map((hit, index) => (
                <ReferenceLine
                  key={index}
                  x={hit.date}
                  stroke="hsl(var(--steelers-gold))"
